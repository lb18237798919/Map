...
export function setOption1(countryAddConfirmRankList) {
	...
	for (let val of countryAddConfirmRankList) {
		xAxis.push(val.nation);//国家
		yAxis.push(val.addConfirm);//确诊人数
	}
	let option1 = {
		color: ["#2f89cf"],
		tooltip: {
...
export function setOption2(foreignList) {
	...
	let rate = [];
	let yAxis2 = [];
	let countryList = [0, 1, 7, 13, 41]//0美国 1西班牙  7加拿大 13韩国 41印度
...
export function setOption4(globalDailyHistory) {
	...
	let series1 = [];
	let series2 = []
	globalDailyHistory = globalDailyHistory.reverse()//翻转数据及坐标轴，每七天生成一个折线点
